...
object SparkWordCount {
  ...
  def setSparkContext(s: SparkContext): Unit = sc = s
  ...
  def input(inputFileName: String): RDD[String] = {
    val sparkConf = new SparkConf().setAppName("Spark WordCount").setMaster("local")
    setSparkContext(new SparkContext(sparkConf))
